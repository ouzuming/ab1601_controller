...
void _LeXport_RxCB(ATT_CB_TYPE type, uint8_t linkindex, uint16_t handle)
{
    ...
    //uart2_utxbuf_write(att_current_Len, TRSPX_Write_Data);
...
{
    static uint8_t ivalue  =0x00;
	if(TRSPX_ble_status == BLE_CONNECTED)
    {
        memset( TRSPX_Read_Data, 0x66,20 );

        TRSPX_Read_Data[0] = ivalue++;
    	BLE_att_set_attribute_length(trspx_notify_handle,len);
        BLE_gatt_charact_notify(trspx_notify_handle,0);
    }
}
